            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.getValue() != null)
                {
                    for(DataSnapshot daySnapshot : snapshot.getChildren())
                    {
                        ...
                        {
                            weekRef.child(Util.getCurrentWeekOfYear()).child("days").child(daySnapshot.getKey()).child("distance").addListenerForSingleValueEvent(new ValueEventListener() {
                                @Override
                                public void onDataChange(@NonNull DataSnapshot snapshot) {
                                    if (snapshot.getValue() != null) {
                                        //changing the pushup value for the current day in the database
                                        weekRef.child(Util.getCurrentWeekOfYear()).child("days").child(daySnapshot.getKey()).child("distance").setValue(snapshot.getValue(Double.class) + distanceOverall);
                                    }
                                    else
                                    {
        ...
        reference.child(userID).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                try {
                    ...
                    userAgility = userProfile.getAgility();
                } catch (Exception e) {
                    e.printStackTrace();
